...
<div class="dev-container">
  <div class="dev-header">
    ...
    <p class="subtitle">Development tools and system diagnostics</p>
  </div>
  ...
  <div class="db-section">
    ...
    <div class="db-stats">
      <div class="db-stat">
        ...
        <span class="stat-value">{data.tableStats.quizzes}</span>
      </div>
      <div class="db-stat">
        <span class="stat-label">Questions:</span>
        <span class="stat-value">{data.tableStats.questions}</span>
      </div>
      <div class="db-stat">
        <span class="stat-label">Attempts:</span>
        <span class="stat-value">{data.tableStats.attempts}</span>
      </div>
    </div>
  </div>
  ...
  <div class="tools-section">
    ...
    <div class="tools-grid">
      <form method="POST" action="?/seedData" use:enhance>
        <button type="submit" class="tool-btn primary">
          🌱 Seed Sample Data
        </button>
        <p class="tool-desc">Add sample quizzes for testing</p>
      </form>
    ...
    <div class="activity-grid">
      <div class="activity-panel">
        ...
        <div class="activity-list">
          {#each data.recentActivity.quizzes as quiz (quiz.id)}
            <div class="activity-item">
              <span class="activity-title">{quiz.topic}</span>
              <span class="activity-time">{formatDate(quiz.createdAt)}</span>
              <button 
                class="copy-btn" 
                on:click={() => copyToClipboard(quiz.id)}
                title="Copy ID"
              >
                📋
              </button>
          ...
          {/each}
          {#if data.recentActivity.quizzes.length === 0}
            <p class="empty-message">No recent quizzes</p>
          {/if}
        </div>
      </div>
      ...
      <div class="activity-panel">
        ...
        <div class="activity-list">
          {#each data.recentActivity.attempts as attempt (attempt.id)}
            <div class="activity-item">
              <span class="activity-title">
                Score: {attempt.score}/{attempt.totalQuestions}
              </span>
              <span class="activity-time">{formatDate(attempt.completedAt)}</span>
              <button 
                class="copy-btn" 
                on:click={() => copyToClipboard(attempt.id)}
                title="Copy ID"
              >
                📋
              </button>
          ...
          {/each}
          {#if data.recentActivity.attempts.length === 0}
            <p class="empty-message">No recent attempts</p>
          {/if}
        </div>
      </div>
    </div>
